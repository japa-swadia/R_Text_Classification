...
ggplot(tdf, aes(x = tdf$Var1, y = tdf$Freq)) +
  ...
  geom_text(aes(label = sprintf("%.1f%%", tdf$Freq/1495 * 100)), 
            vjust = -.5)

# Data visualization for training data
trainsample <- train_df[2:332, ]
testsample <- train_df[333:672, ]
tdf1.test <- data.frame(table(testsample$Requirement.Type))
tdf1 <- data.frame(table(trainsample$Requirement.Type))
ggplot(tdf, aes(x = tdf1$Var1, y = tdf1$Freq)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = sprintf("%.2f%%", tdf1$Freq/332 * 100)), 
            vjust = -.8)
...
knn1.pred <- knn1(model_data[train_set, ], model_data[test_set, ], classifier[train_set], k=5)
...
ggplot(tdf, aes(x = tdf2$Var1, y = tdf2$Freq)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = sprintf("%.2f%%", tdf2$Freq/340 * 100)), 
            vjust = -.5)

# Produce a csv file of predicted output
write.csv(output.pred, file="C:/Users/japas_000/Documents/Thesis/model/knn_output2.csv")
